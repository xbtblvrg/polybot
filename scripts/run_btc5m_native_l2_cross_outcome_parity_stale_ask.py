#!/usr/bin/env python3
"""BTC-native receipt-clock cross-outcome parity into a stale ask."""
...
FORWARD_START_S = 1_785_009_900  # 2026-07-25T20:05:00Z
ORDER_USD = 1.0
MIN_PRICE, MAX_PRICE = 0.25, 0.50
ENTRY_START_S, LAST_ENTRY_S = 30, 180
MIN_DUAL_BOOK_OVERLAP_S = 0.0
MIN_NET_CROSS_EDGE_PER_SHARE = 0.0
MAX_RECEIPT_GAP_S = 5.0
SLIPPAGE_PER_SHARE = 0.005
ADVERSE_PER_SHARE = 0.010
ZERO_INTENT_WINDOWS, MAX_WINDOWS, MIN_RESOLVED = 2, 6, 10
STATE = "data/research/btc5m_native_l2_cross_outcome_parity_stale_ask_state.json"
CACHE = "data/research/btc5m_native_l2_cross_outcome_parity_stale_ask_raw_cache.json"
TERMINALS = "data/research/btc5m_native_l2_cross_outcome_parity_stale_ask_terminals.jsonl"
EVENTS = "data/research/btc5m_native_l2_cross_outcome_parity_stale_ask_events.jsonl"
SELECTOR = "data/research/btc5m_native_l2_cross_outcome_parity_stale_ask_selector.json"
...
CONFIG = {
    "schema_version": 1,
    "method": "btc5m_native_l2_cross_outcome_parity_stale_ask_v1",
    "economic_edge": "native_receipt_clock_cross_outcome_parity_fair_vs_same_outcome_ask",
    "market_identity": {
        "target": "btc-updown-5m-{window}",
        "outcomes": ["Up", "Down"],
    },
    "source_schema": "immutable_gap_bounded_dual_outcome_l2_receipts_v1",
    "calibration_method": "complement_microprice_parity_to_executable_same_outcome_ask",
    "training_cutoff_s": 1_785_009_300,
    "forward_start_s": FORWARD_START_S,
    "features": {
        "min_dual_book_overlap_s": MIN_DUAL_BOOK_OVERLAP_S,
        "min_net_cross_edge_per_share": MIN_NET_CROSS_EDGE_PER_SHARE,
        "max_receipt_gap_s": MAX_RECEIPT_GAP_S,
        "min_executable_depth_usd": ORDER_USD,
    },
    ...
    "signal_rule": "fair_outcome_equals_one_minus_complement_microprice; strictly_positive_post_cost_edge; higher_net_edge_wins; exact_equal_edge_abstains",
    "matched_baseline": "same_cross_outcome_parity_event_windows_with_immutable_empty_btc_cash_flows",
    "terminal_clock": {
...
CELL_ID = f"l2_cross_parity_{CHECKSUM[:12]}"
PREREG = f"data/research/btc5m_native_l2_cross_outcome_parity_stale_ask_preregistration_{CHECKSUM[:12]}.json"
...
def preregister() -> dict[str, Any]:
    body = {
        **CONFIG,
        "kind": "btc5m_native_l2_cross_outcome_parity_stale_ask_preregistration",
...
def choose_signal(
    *,
    ...
    for outcome in ("Up", "Down"):
        current = (books.get("BTC") or {}).get(outcome) or {}
        complement = "Down" if outcome == "Up" else "Up"
        complement_book = (books.get("BTC") or {}).get(complement) or {}
        ask, depth = _num(current.get("best_ask")), _num(current.get("best_ask_size"))
        shares = ORDER_USD / ask if ask > 0 else 0.0
        fee = expected_polymarket_buy_fee_usd(shares=shares, price=ask)
        p_fair = min(1.0, max(0.0, 1.0 - _num(complement_book.get("microprice"))))
        reserve = (fee / shares if shares else 99.0) + SLIPPAGE_PER_SHARE + ADVERSE_PER_SHARE
        edge = p_fair - ask - reserve
        local: list[str] = []
        if current.get("status") != "PASS" or complement_book.get("status") != "PASS":
            local.append("gap_bounded_dual_book_missing")
        ...
        if edge <= MIN_NET_CROSS_EDGE_PER_SHARE:
        ...
        candidates.append({
            ...
            "gross_edge_per_share": round(p_fair - ask, 6),
            "executable_ask": ask,
    ...
    if len(ranked) > 1 and ranked[0]["net_edge_per_share"] == ranked[1]["net_edge_per_share"]:
        return None, ["dual_outcome_equal_edge"]
...
def build_intent(signal: dict[str, Any], market: dict[str, Any], now: float) -> dict[str, Any]:
    ...
    return CopyIntent(
        intent_id=stable_id("crossparityci", {"generation": CHECKSUM, "market": slug}),
        source_wallet=f"BTC5M_PROMOTED_CELL:{CELL_ID}",
        wallet_name=CELL_ID,
        source_event_id=stable_id("crossparitye", {"market": slug, "event_ts": signal["event_ts"], "outcome": outcome}),
        ...
        policy_id="native_l2_cross_outcome_parity_stale_ask_costed_cap1",
        ...
        live_orders_allowed=False,
        reason="native receipt-clock cross-outcome parity fair before ask reprice",
...
def reduce_generation(terminals: list[dict[str, Any]], events: list[dict[str, Any]], resolutions: dict[str, str]) -> dict[str, Any]:
    ...
    rows.sort(key=lambda row: int(row.get("window_start_s") or 0))
    ...
    if pnl and sum(pnl) < 0:
        status = "PARK_NEGATIVE_ROLLING_PAPER_PNL"
    elif any(integrity.values()) or parity:
        status = "PARK_IRRECOVERABLE_INTEGRITY_DISAGREEMENT"
    elif len(rows) >= ZERO_INTENT_WINDOWS and all(
        not isinstance(row.get("intent"), dict)
        for row in rows[-ZERO_INTENT_WINDOWS:]
    ):
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    terminal_id = stable_id("crossparityterm", {"generation": CHECKSUM, "window": window})
    ...
    try:
        ...
        errors.append(f"native_l2_fetch:{type(exc).__name__}")
    ...
    if window >= FORWARD_START_S and current_terminal is None and existing_event is None and markets and ENTRY_START_S <= elapsed <= LAST_ENTRY_S:
        ...
        if signal:
            ...
            event = {
                "schema_version": 1, "event": "btc5m_native_l2_cross_outcome_parity_stale_ask_paper_fill",
    ...
    if window >= FORWARD_START_S and current_terminal is None and elapsed >= 270:
        ...
        terminal = {
            "schema_version": 1, "event": "btc5m_native_l2_cross_outcome_parity_stale_ask_terminal",
    ...
    atomic_write_json(_rooted(args.cache), {
        "schema_version": 1, "kind": "btc5m_native_l2_cross_outcome_parity_stale_ask_raw_cache",
    ...
    payload = {
        "schema_version": 1, "kind": "btc5m_native_l2_cross_outcome_parity_stale_ask_generation",

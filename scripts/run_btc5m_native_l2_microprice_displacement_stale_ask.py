#!/usr/bin/env python3
"""BTC-native receipt-clock L2 microprice displacement into a stale ask."""
...
FORWARD_START_S = 1_785_007_800  # 2026-07-25T19:30:00Z
ORDER_USD = 1.0
...
STATE = "data/research/btc5m_native_l2_microprice_displacement_stale_ask_state.json"
CACHE = "data/research/btc5m_native_l2_microprice_displacement_stale_ask_raw_cache.json"
TERMINALS = "data/research/btc5m_native_l2_microprice_displacement_stale_ask_terminals.jsonl"
EVENTS = "data/research/btc5m_native_l2_microprice_displacement_stale_ask_events.jsonl"
SELECTOR = "data/research/btc5m_native_l2_microprice_displacement_stale_ask_selector.json"
...
CONFIG = {
    "schema_version": 1,
    "method": "btc5m_native_l2_microprice_displacement_stale_ask_v1",
    "economic_edge": "native_receipt_clock_l2_microprice_displacement_before_resting_ask_reprice",
    "market_identity": {
        "target": "btc-updown-5m-{window}",
        "outcomes": ["Up", "Down"],
    },
    "source_schema": "immutable_gap_bounded_dual_outcome_l2_receipts_v1",
    "calibration_method": "rolling_receipt_clock_microprice_displacement_to_stale_executable_ask",
    "training_cutoff_s": 1_785_007_200,
    ...
    "signal_rule": "receipt_clock_dual_l2_microprice_displacement_to_stale_same_outcome_ask; higher_net_edge_wins; exact_equal_edge_abstains",
    "matched_baseline": "same_l2_displacement_event_windows_with_immutable_empty_btc_cash_flows",
    "terminal_clock": {
...
CELL_ID = f"l2_displacement_{CHECKSUM[:12]}"
PREREG = f"data/research/btc5m_native_l2_microprice_displacement_stale_ask_preregistration_{CHECKSUM[:12]}.json"
...
def preregister() -> dict[str, Any]:
    body = {
        **CONFIG,
        "kind": "btc5m_native_l2_microprice_displacement_stale_ask_preregistration",
...
def choose_signal(
    *,
    ...
    for outcome in ("Up", "Down"):
        ...
        micro_move = _num(current.get("microprice")) - _num(prior.get("microprice"))
        ask_reprice = abs(_num(current.get("best_ask")) - _num(prior.get("best_ask")))
        ...
        if micro_move < MIN_MICROPRICE_MOVE:
            local.append("microprice_displacement_below_0p008")
...
def build_intent(signal: dict[str, Any], market: dict[str, Any], now: float) -> dict[str, Any]:
    ...
    return CopyIntent(
        intent_id=stable_id("l2dispci", {"generation": CHECKSUM, "market": slug}),
        source_wallet=f"BTC5M_PROMOTED_CELL:{CELL_ID}",
        wallet_name=CELL_ID,
        source_event_id=stable_id("l2dispe", {"market": slug, "event_ts": signal["event_ts"], "outcome": outcome}),
        ...
        policy_id="native_l2_microprice_displacement_stale_ask_costed_cap1",
        ...
        live_orders_allowed=False,
        reason="native receipt-clock L2 microprice displacement before ask reprice",
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    terminal_id = stable_id("l2dispterm", {"generation": CHECKSUM, "window": window})
    ...
    if window >= FORWARD_START_S and current_terminal is None and existing_event is None and markets and ENTRY_START_S <= elapsed <= LAST_ENTRY_S:
        ...
        if signal:
            ...
            event = {
                "schema_version": 1, "event": "btc5m_native_l2_microprice_displacement_stale_ask_paper_fill",
    ...
    if window >= FORWARD_START_S and current_terminal is None and elapsed >= 270:
        ...
        terminal = {
            "schema_version": 1, "event": "btc5m_native_l2_microprice_displacement_stale_ask_terminal",
    ...
    atomic_write_json(_rooted(args.cache), {
        "schema_version": 1, "kind": "btc5m_native_l2_microprice_displacement_stale_ask_raw_cache",
    ...
    payload = {
        "schema_version": 1, "kind": "btc5m_native_l2_microprice_displacement_stale_ask_generation",

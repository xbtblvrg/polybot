#!/usr/bin/env python3
"""BTC-native receipt-clock L2 top-of-book pressure acceleration."""
...
FORWARD_START_S = 1_785_008_700  # 2026-07-25T19:45:00Z
ORDER_USD = 1.0
MIN_PRICE, MAX_PRICE = 0.25, 0.50
ENTRY_START_S, LAST_ENTRY_S = 30, 180
LOOKBACK_S = 3.0
MIN_ABS_PRESSURE = 0.45
MIN_PRESSURE_DELTA = 0.20
MAX_RECEIPT_GAP_S = 5.0
SLIPPAGE_PER_SHARE = 0.005
ADVERSE_PER_SHARE = 0.010
ZERO_INTENT_WINDOWS, MAX_WINDOWS, MIN_RESOLVED = 2, 6, 10
STATE = "data/research/btc5m_native_l2_tob_pressure_imbalance_state.json"
CACHE = "data/research/btc5m_native_l2_tob_pressure_imbalance_raw_cache.json"
TERMINALS = "data/research/btc5m_native_l2_tob_pressure_imbalance_terminals.jsonl"
EVENTS = "data/research/btc5m_native_l2_tob_pressure_imbalance_events.jsonl"
SELECTOR = "data/research/btc5m_native_l2_tob_pressure_imbalance_selector.json"
...
CONFIG = {
    "schema_version": 1,
    "method": "btc5m_native_l2_tob_pressure_imbalance_v1",
    "economic_edge": "native_receipt_clock_top_of_book_size_pressure_acceleration",
    "market_identity": {
        "target": "btc-updown-5m-{window}",
        "outcomes": ["Up", "Down"],
    },
    "source_schema": "immutable_gap_bounded_dual_outcome_l2_receipts_v1",
    "calibration_method": "rolling_receipt_clock_top_of_book_pressure_and_acceleration",
    "training_cutoff_s": 1_785_008_100,
    "forward_start_s": FORWARD_START_S,
    "features": {
        "lookback_s": LOOKBACK_S,
        "min_abs_pressure": MIN_ABS_PRESSURE,
        "min_pressure_delta_over_lookback": MIN_PRESSURE_DELTA,
    ...
    "signal_rule": "receipt_clock_dual_l2_top_pressure_gte_0p45_and_delta_gte_0p20; higher_net_edge_wins; exact_equal_edge_abstains",
    "matched_baseline": "same_l2_pressure_event_windows_with_immutable_empty_btc_cash_flows",
    "terminal_clock": {
...
CELL_ID = f"l2_tob_pressure_{CHECKSUM[:12]}"
PREREG = f"data/research/btc5m_native_l2_tob_pressure_imbalance_preregistration_{CHECKSUM[:12]}.json"
...
def preregister() -> dict[str, Any]:
    body = {
        **CONFIG,
        "kind": "btc5m_native_l2_tob_pressure_imbalance_preregistration",
...
def choose_signal(
    *,
    ...
    for outcome in ("Up", "Down"):
        ...
        prior = _prior_book(snapshots, "BTC", outcome, now - LOOKBACK_S)
        current_total = _num(current.get("best_bid_size")) + _num(current.get("best_ask_size"))
        prior_total = _num(prior.get("best_bid_size")) + _num(prior.get("best_ask_size"))
        pressure = (
            (_num(current.get("best_bid_size")) - _num(current.get("best_ask_size"))) / current_total
            if current_total > 0 else 0.0
        )
        prior_pressure = (
            (_num(prior.get("best_bid_size")) - _num(prior.get("best_ask_size"))) / prior_total
            if prior_total > 0 else 0.0
        )
        pressure_delta = pressure - prior_pressure
        ...
        p_fair = _num(current.get("microprice"))
        reserve = (fee / shares if shares else 99.0) + SLIPPAGE_PER_SHARE + ADVERSE_PER_SHARE
        edge = p_fair - ask - reserve
        local: list[str] = []
        if pressure < MIN_ABS_PRESSURE:
            local.append("tob_pressure_below_0p45")
        if pressure_delta < MIN_PRESSURE_DELTA:
            local.append("tob_pressure_delta_below_0p20")
        if prior.get("status") != "PASS" or current.get("status") != "PASS":
        ...
        candidates.append({
            ...
            "pressure_lookback": round(prior_pressure, 6),
...
def build_intent(signal: dict[str, Any], market: dict[str, Any], now: float) -> dict[str, Any]:
    ...
    return CopyIntent(
        intent_id=stable_id("tobpressureci", {"generation": CHECKSUM, "market": slug}),
        source_wallet=f"BTC5M_PROMOTED_CELL:{CELL_ID}",
        wallet_name=CELL_ID,
        source_event_id=stable_id("tobpressuree", {"market": slug, "event_ts": signal["event_ts"], "outcome": outcome}),
        ...
        policy_id="native_l2_tob_pressure_imbalance_costed_cap1",
        ...
        live_orders_allowed=False,
        reason="native receipt-clock L2 top-of-book pressure acceleration",
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    terminal_id = stable_id("tobpressureterm", {"generation": CHECKSUM, "window": window})
    ...
    if window >= FORWARD_START_S and current_terminal is None and existing_event is None and markets and ENTRY_START_S <= elapsed <= LAST_ENTRY_S:
        ...
        if signal:
            ...
            event = {
                "schema_version": 1, "event": "btc5m_native_l2_tob_pressure_imbalance_paper_fill",
    ...
    if window >= FORWARD_START_S and current_terminal is None and elapsed >= 270:
        ...
        terminal = {
            "schema_version": 1, "event": "btc5m_native_l2_tob_pressure_imbalance_terminal",
    ...
    atomic_write_json(_rooted(args.cache), {
        "schema_version": 1, "kind": "btc5m_native_l2_tob_pressure_imbalance_raw_cache",
    ...
    payload = {
        "schema_version": 1, "kind": "btc5m_native_l2_tob_pressure_imbalance_generation",

#!/usr/bin/env python3
"""Native ETH+SOL leader-lag into BTC-5m (paper only)."""
...
FORWARD_START_S = 1_785_003_600  # 2026-07-25T18:20:00Z
ORDER_USD = 1.0
...
STATE = "data/research/btc5m_polymarket_cross_asset_leader_lag_state.json"
CACHE = "data/research/btc5m_polymarket_cross_asset_leader_lag_raw_cache.json"
TERMINALS = "data/research/btc5m_polymarket_cross_asset_leader_lag_terminals.jsonl"
EVENTS = "data/research/btc5m_polymarket_cross_asset_leader_lag_events.jsonl"
SELECTOR = "data/research/btc5m_polymarket_cross_asset_leader_lag_selector.json"
...
CONFIG = {
    "schema_version": 1,
    "method": "btc5m_polymarket_cross_asset_leader_lag_v1",
    "economic_edge": "native_eth_sol_joint_price_discovery_before_btc_outcome_reprice",
    "market_identity": {
    ...
    "calibration_method": "joint_leader_same_direction_mean_outcome_microprice",
    "training_cutoff_s": 1_785_003_300,
    ...
    "features": {
        ...
        "two_leader_max_alignment_s": MAX_ALIGNMENT_S,
    ...
    "matched_baseline": "same_joint_leader_windows_with_immutable_empty_btc_cash_flows",
    "terminal_clock": {
...
CELL_ID = f"cross_asset_{CHECKSUM[:12]}"
PREREG = f"data/research/btc5m_polymarket_cross_asset_leader_lag_preregistration_{CHECKSUM[:12]}.json"
...
def preregister() -> dict[str, Any]:
    body = {
        **CONFIG,
        "kind": "btc5m_polymarket_cross_asset_leader_lag_preregistration",
    ...
    *,
    ...
    for outcome in ("Up", "Down"):
        ...
        if len(leaders) != 2:
            ...
            continue
        alignment = abs(_num(leaders[0]["event_ts"]) - _num(leaders[1]["event_ts"]))
        btc_book = (books.get("BTC") or {}).get(outcome) or {}
        btc_prior = _prior_book(snapshots, "BTC", outcome, min(_num(row["event_ts"]) for row in leaders))
        btc_reprice = abs(_num(btc_book.get("microprice")) - _num(btc_prior.get("microprice")))
        mean_move = sum(_num(row["microprice_displacement"]) for row in leaders) / 2
        ...
        candidates.append({
            "outcome": outcome,
            "leaders": leaders,
            "leader_alignment_s": round(alignment, 6),
...
def build_intent(signal: dict[str, Any], market: dict[str, Any], now: float) -> dict[str, Any]:
    ...
    return CopyIntent(
        intent_id=stable_id("xassetci", {"generation": CHECKSUM, "market": slug}),
        source_wallet=f"BTC5M_PROMOTED_CELL:{CELL_ID}",
        wallet_name=CELL_ID,
        source_event_id=stable_id("xassete", {"market": slug, "leaders": signal["leaders"]}),
        ...
        policy_id="native_cross_asset_joint_leader_costed_cap1",
        ...
        live_orders_allowed=False,
        reason="joint native ETH+SOL leader sweep before BTC reprice",
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    terminal_id = stable_id("xassetterm", {"generation": CHECKSUM, "window": window})
    ...
    try:
        ...
        errors.append(f"native_cross_asset_fetch:{type(exc).__name__}")
    ...
    if window >= FORWARD_START_S and current_terminal is None and existing_event is None and markets and ENTRY_START_S <= elapsed <= LAST_ENTRY_S:
        ...
        if signal:
            ...
            event = {
                "schema_version": 1, "event": "btc5m_cross_asset_leader_lag_paper_fill",
    ...
    if window >= FORWARD_START_S and current_terminal is None and elapsed >= 270:
        ...
        terminal = {
            "schema_version": 1, "event": "btc5m_polymarket_cross_asset_leader_lag_terminal",
    ...
    atomic_write_json(_rooted(args.cache), {
        "schema_version": 1, "kind": "btc5m_polymarket_cross_asset_leader_lag_raw_cache",
    ...
    payload = {
        "schema_version": 1, "kind": "btc5m_polymarket_cross_asset_leader_lag_generation",

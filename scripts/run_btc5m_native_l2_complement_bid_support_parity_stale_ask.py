#!/usr/bin/env python3
"""BTC-native complement best-bid support parity into a stale ask."""
...
FORWARD_START_S = 1_785_012_600  # 2026-07-25T20:50:00Z
STATE = "data/research/btc5m_native_l2_complement_bid_support_parity_stale_ask_state.json"
CACHE = "data/research/btc5m_native_l2_complement_bid_support_parity_stale_ask_raw_cache.json"
TERMINALS = "data/research/btc5m_native_l2_complement_bid_support_parity_stale_ask_terminals.jsonl"
EVENTS = "data/research/btc5m_native_l2_complement_bid_support_parity_stale_ask_events.jsonl"
SELECTOR = "data/research/btc5m_native_l2_complement_bid_support_parity_stale_ask_selector.json"

CONFIG = {
    **base.CONFIG,
    "method": "btc5m_native_l2_complement_bid_support_parity_stale_ask_v1",
    "economic_edge": "native_receipt_clock_complement_bid_support_fair_vs_same_outcome_ask",
    "calibration_method": "one_minus_complement_best_bid_to_executable_same_outcome_ask",
    "training_cutoff_s": FORWARD_START_S - 600,
    "forward_start_s": FORWARD_START_S,
    "signal_rule": "fair_outcome_equals_one_minus_complement_best_bid; strictly_positive_post_cost_edge; higher_net_edge_wins; exact_equal_edge_abstains",
    "terminal_clock": {
...
CELL_ID = f"l2_complement_bid_support_{CHECKSUM[:12]}"
PREREG = (
    "data/research/"
    f"btc5m_native_l2_complement_bid_support_parity_stale_ask_preregistration_{CHECKSUM[:12]}.json"
)
...
def choose_signal(**kwargs: Any) -> tuple[dict[str, Any] | None, list[str]]:
    ...
    for asset, outcomes in books.items():
        projected[asset] = {
            outcome: {**book, "microprice": book.get("best_bid")}
    ...
    if signal:
        ...
        signal["fair_basis"] = "one_minus_complement_best_bid"
        signal["complement_best_bid"] = complement_book.get("best_bid")
...
def build_intent(signal: dict[str, Any], market: dict[str, Any], now: float) -> dict[str, Any]:
    ...
    intent["policy_id"] = "native_l2_complement_bid_support_parity_stale_ask_costed_cap1"
    intent["reason"] = "native receipt-clock complement bid support fair before ask reprice"
    return intent

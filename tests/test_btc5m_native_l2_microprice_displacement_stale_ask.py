from scripts import run_btc5m_native_l2_microprice_displacement_stale_ask as lane
...
def test_generation_is_frozen_paper_only():
    ...
    assert lane.CONFIG["method"] == "btc5m_native_l2_microprice_displacement_stale_ask_v1"
    assert lane.CONFIG["forward_start_s"] == 1_785_007_800
    assert lane.CONFIG["source_schema"] == "immutable_gap_bounded_dual_outcome_l2_receipts_v1"
    assert lane.CONFIG["paper_only"] is True
    assert lane.CONFIG["live_orders_allowed"] is False


def test_l2_displacement_emits_without_trade_tape():
    now = 100.0
    snapshots = [
        {
            "asset_name": "BTC",
    ...
    assert signal["microprice_displacement"] == 0.02
    assert signal["ask_reprice"] == 0.005
    ...
    terminals = [
        {
            ...
            "raw_integrity": {},
            "blockers": ["microprice_displacement_below_0p008"],
        }
        for offset in (0, 300)
    ]
